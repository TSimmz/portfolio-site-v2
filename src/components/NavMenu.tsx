    ...
    >
      {navLinks.map((link: NavLink) => {
        ...
        return (
          <div
            key={link.title}
            className="relative flex flex-row space-x-0 rounded pr-10 transition-all duration-500 hover:text-rose-500 hover:backdrop-brightness-125"
          >
            <Link href={link.href} className="flex align-middle">
              <span className="px-2 py-1">{link.title}</span>
            </Link>
            <div
